class FontScannerApp {
  ...
  initializeSocketHandlers() {
    ...
    this.socket.on('progress', (data) => {
      console.log('C Progress update:', data);
      this.handleProgressUpdate(data);
    });
  }
  
  handleProgressUpdate(data) {
    console.log('C Progress update received:', data);
    const { step, total, name, status, progress, elapsed, remaining } = data;
    
    // Update progress UI
    const progressContainer = document.getElementById('progressContainer');
    console.log('R Progress container exists:', !!progressContainer);
    ...
    if (status === 'running') {
      ...
      // Ensure consistent icon color for running state (amber)
  ...
  showProgressUI() {
    console.log('Y showProgressUI() called');
    // Create progress container if it doesn't exist
    let progressContainer = document.getElementById('progressContainer');
    console.log('R Existing container:', progressContainer);
  ...
  hideProgressUI() {
    ...
    const progressContainer = document.getElementById('progressContainer');
    console.log('R Progress container found:', !!progressContainer);
    if (progressContainer) {
      progressContainer.style.display = 'none';
      console.log('✓ Progress container display set to none');
    } else {
      console.warn('~ Progress container not found when trying to hide');
    }
  }

  generateProgressSteps() {
    const steps = [
      { id: 1, name: 'Fonts', icon: 'F' },
      { id: 2, name: 'Performance', icon: 'P' },
      { id: 3, name: 'Practices', icon: 'B' },
      { id: 4, name: 'AI', icon: 'A' },
      { id: 5, name: 'Metrics', icon: 'M' },
      { id: 6, name: 'Browser', icon: 'X' },
      { id: 7, name: 'A11y', icon: 'A' },
      { id: 8, name: 'License', icon: 'L' },
      { id: 9, name: 'Bench', icon: 'B' },
      { id: 10, name: 'Lighthouse', icon: 'H' }
    ];
  ...
  initializeElements() {
    console.log('S Finding DOM elements...');
  ...
  initializeFontSizeControls() {
    ...
    console.log(`C Found ${fontSizeButtons.length} font size buttons`);
    
    if (fontSizeButtons.length === 0) {
      console.warn('~ No font size buttons found in DOM!');
      return;
    }
  ...
  bindEvents() {
    console.log('K Binding events to elements...');
    if (this.analyzeButton) {
      this.analyzeButton.addEventListener('click', () => {
        console.log('S Analyze button clicked!');
        this.handleScan();
      });
  ...
  async handleScan() {
    ...
    console.log('S Starting scan...', { url });
  ...
  async performComprehensiveScan(normalizedUrl) {
    console.log('T performComprehensiveScan called with URL:', normalizedUrl);
...
/_/   /____//___/ /_/ |_|/___/ /___/    /____//___/    /_/   /_/ |_| /_/   /___/ /___/ /_/|_/   /_/    </pre>
    ...
    try {
      ...
      setTimeout(() => {
        console.log('R Full response data:', data);
        console.log('S data.success:', data.success);
        console.log('S data.data exists:', !!data.data);
  ...
  prepareDataForDisplay(data) {
    console.log('T Preparing comprehensive results for display:', data);
  ...
  displayComprehensiveResults(data) {
    ...
    setTimeout(() => {
      console.log('T Grade Debug - data.overallScore:', data.overallScore);
      console.log('T Grade Debug - data.grade:', data.grade);
      console.log('T Grade Debug - transformedData.results.overallScore:', transformedData.results.overallScore);
      console.log('T Grade Debug - transformedData.results.grade:', transformedData.results.grade);
      ...
      if (this.resultsContainer && (data.overallScore || transformedData.results.overallScore) && (data.grade || transformedData.results.grade)) {
        ...
        gradeIndicator.style.cssText = `
          background: ${isLightMode ? '#ffffff' : 'linear-gradient(135deg, #00ff41 0%, #00cc33 100%)'};
          border: 2px solid ${isLightMode ? '#00994d' : '#00ff41'};
          border-radius: 8px;
          padding: 1.5rem;
          margin-bottom: 1.5rem;
          text-align: center;
          box-shadow: ${isLightMode ? '0 2px 8px rgba(0, 0, 0, 0.1)' : '0 0 20px rgba(0, 255, 65, 0.4)'};
          position: relative;
          overflow: hidden;
        `;
        ...
        const textColor = isLightMode ? '#1a1a1a' : '#000';
        const secondaryOpacity = isLightMode ? '0.7' : '0.8';
        const tertiaryOpacity = isLightMode ? '0.6' : '0.7';
        
        gradeIndicator.innerHTML = `
          <div class="grade-badge">
            <div style="color: ${textColor}; font-weight: 700; font-size: 1rem; margin-bottom: 0.5rem; text-shadow: none;">
              → COMPREHENSIVE ANALYSIS COMPLETE
            </div>
            <div style="color: ${textColor}; font-size: 2.5rem; font-weight: 900; text-shadow: none; margin: 0.5rem 0;">
              GRADE: ${finalGrade}
            </div>
            <div style="color: ${textColor}; opacity: ${secondaryOpacity}; font-size: 1.2rem; font-weight: 600; text-shadow: none;">
              SCORE: ${finalScore}/100
            </div>
            <div style="color: ${textColor}; opacity: ${tertiaryOpacity}; font-size: 0.9rem; margin-top: 0.5rem; text-shadow: none;">
      ...
      if (currentStep < progressSteps.length) {
        ...
        progressText.innerHTML = `
          ...
          <span style="font-size: 0.9rem; color: #00ff41; opacity: 0.9; letter-spacing: 0.05em;">
            ⧗ Elapsed: <strong>${elapsed}s</strong> | ⧗ Est. remaining: <strong>${remaining}s</strong>
          </span>
        `;
  ...
  loadWebFonts(results) {
    ...
    if (!results || !results.fonts) {
      console.log('~ No fonts found in results');
      return;
    }
    ...
    if (cssContent) {
      ...
      console.log('~ No web fonts to load');
    }
  }
  ...
  displayResults(data) {
    console.log('Y displayResults called with:', data);
    ...
    console.log('S DEBUG displayResults received results:', results);
    console.log('S DEBUG displayResults results.fonts:', results.fonts);
    console.log('S DEBUG displayResults results.fonts type:', typeof results.fonts);
    console.log('S DEBUG displayResults results.fonts keys:', Object.keys(results.fonts || {}));
    ...
    // Lighthouse Analysis - only show if actual data exists (moved to Core Web Vitals page)
    // Check if lighthouse has real data with actual scores, not just null/undefined/error placeholders
    const hasLighthouseData = results.lighthouse &&
      ((results.lighthouse.desktop && !results.lighthouse.desktop.error && results.lighthouse.desktop.performance > 0) ||
       (results.lighthouse.mobile && !results.lighthouse.mobile.error && results.lighthouse.mobile.performance > 0));

    if (hasLighthouseData) {
      console.log('✓ Lighthouse data detected, showing section');
      ...
      console.log('ℹ️ No Lighthouse data available - section hidden (Lighthouse moved to Core Web Vitals page)');
    }
  ...
  createExecutiveSummary(results, url, scannedAt) {
    console.log('T Executive Summary Debug - Full results object:', results);
    
    const section = document.createElement('div');
    section.className = 'section executive-summary';

    // Debug lighthouse data structure
    console.log('T Lighthouse data available:', {
    ...
    // Enhanced debugging for performance scores
    console.log('T Desktop Performance Data:', {
      path1: results.lighthouse?.desktop?.performance,
      path2: results.lighthouse?.desktop?.score,
      failed: results.lighthouse?.desktop?.failed,
      error: results.lighthouse?.desktop?.error,
      fullDesktop: results.lighthouse?.desktop
    });
    console.log('T Mobile Performance Data:', {
      path1: results.lighthouse?.mobile?.performance,
      path2: results.lighthouse?.mobile?.score,
      failed: results.lighthouse?.mobile?.failed,
      error: results.lighthouse?.mobile?.error,
      fullMobile: results.lighthouse?.mobile
    });

    // Check if Lighthouse failed for desktop or mobile
    const desktopFailed = results.lighthouse?.desktop?.failed || false;
    const mobileFailed = results.lighthouse?.mobile?.failed || false;
    const desktopEstimated = results.lighthouse?.desktop?.estimated || false;
    const desktopUsedMobileFallback = results.lighthouse?.desktop?.usedMobileFallback || false;
    const mobileEstimated = results.lighthouse?.mobile?.estimated || false;

    if (desktopFailed) {
      console.warn('⚠️ Desktop Lighthouse analysis failed:', results.lighthouse?.desktop?.error);
      if (desktopEstimated) {
        console.info('ℹ️ Using estimated scores for desktop');
      }
      if (desktopUsedMobileFallback) {
        console.info('📱 Using mobile scores as fallback for desktop');
      }
    }
    if (mobileFailed) {
      console.warn('⚠️ Mobile Lighthouse analysis failed:', results.lighthouse?.mobile?.error);
      if (mobileEstimated) {
        console.info('ℹ️ Using estimated scores for mobile');
      }
    }

    // Calculate combined performance score (weighted average: mobile 60%, desktop 40%)
    // Get performance scores even if they're estimated/fallback
    const desktopPerf = Math.round(
      results.lighthouse?.desktop?.performance ||
      results.lighthouse?.desktop?.score ||
      0
    );
    const mobilePerf = Math.round(
      results.lighthouse?.mobile?.performance ||
      results.lighthouse?.mobile?.score ||
      0
    );

    console.log('T Calculated Performance Scores - Desktop:', desktopPerf, 'Mobile:', mobilePerf);
    let overallScore = 0;
    ...
    if (desktopPerf > 0 && mobilePerf > 0) {
      ...
      console.log('T Generated performance score from load time:', overallScore);
    } else {
      // Use demo values if no real data
      overallScore = 85;
      console.log('T Using demo performance score:', overallScore);
    }
    ...
    console.log('T Calculated scores - Performance:', overallScore, 'Accessibility:', accessibility, 'SEO:', seo);
    
    // Improved best practices score calculation with better debugging
    let compliance = 0;
    console.log('S Best Practices Debug - Full Results:', {
    ...
    if (results.bestPractices?.score !== undefined && results.bestPractices.score !== null) {
      ...
      console.log('T Generated best practices score based on font count:', fontCount, '=> Score:', compliance);
    }
    
    // Final safeguard - if still 0, use a reasonable default
    if (compliance === 0) {
      compliance = 70;
      console.log('~ Best practices score was 0, using default:', compliance);
    }
    ...
    section.innerHTML = `
      ...
      <div class="summary-intro">
        ...
        <div class="scan-metadata">
          <span class="scan-info">C Analysis completed on ${new Date(scannedAt).toLocaleDateString()} at ${new Date(scannedAt).toLocaleTimeString()}</span>
          <span class="scan-info">S Scan type: Comprehensive multi-page analysis</span>
          <span class="scan-info">P Processing time: ${this.calculateProcessingTime(results, scannedAt)}s</span>
        </div>
        ${performanceSummary}
      </div>
      ...
      <div class="benchmark-status">
        <div class="benchmark-content">
          <div>
            <h4 class="benchmark-title">C Analysis Completeness</h4>
      ...
      <!-- 4-Column Metrics Dashboard with Circular Graphs -->
      <div class="executive-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 2rem; margin: 2rem 0;">
        ${this.createCircularScoreCard('Page Speed', overallScore, 'performance', 'P')}
        ${this.createCircularScoreCard('Best Practices', compliance, 'practices', '✓')}
        ${this.createCircularScoreCard('Accessibility', accessibility, 'accessibility', 'A')}
        ${this.createCircularScoreCard('SEO Score', seo, 'seo', 'S')}
      </div>
      ...
      <div class="secondary-metrics">
        ...
        <div class="metric-item">
          ...
          <span class="metric-label">Mobile Score${mobileEstimated ? ' (Est.)' : ''}</span>
          <span class="metric-value" ${
            mobileEstimated ? 'style="color: #ffb366;" title="Estimated score (Lighthouse unavailable)"' :
            mobileFailed ? 'style="color: #ff6b6b;" title="' + (results.lighthouse?.mobile?.error || 'Analysis failed') + '"' : ''
          }>${
            mobilePerf > 0 ? `${mobilePerf}/100${mobileEstimated ? '*' : ''}` :
            mobileFailed && !mobileEstimated ? 'Failed' : 'N/A'
          }</span>
        </div>
        <div class="metric-item">
          <span class="metric-icon">D</span>
          <span class="metric-label">Desktop Score${desktopEstimated || desktopUsedMobileFallback ? ' (Est.)' : ''}</span>
          <span class="metric-value" ${
            desktopUsedMobileFallback ? 'style="color: #4a9eff;" title="Using mobile scores as fallback"' :
            desktopEstimated ? 'style="color: #ffb366;" title="Estimated score (Lighthouse unavailable)"' :
            desktopFailed ? 'style="color: #ff6b6b;" title="' + (results.lighthouse?.desktop?.error || 'Analysis failed') + '"' : ''
          }>${
            desktopPerf > 0 ? `${desktopPerf}/100${desktopUsedMobileFallback ? '📱' : desktopEstimated ? '*' : ''}` :
            desktopFailed && !desktopEstimated ? 'Failed' : 'N/A'
          }</span>
        </div>
      </div>
    `;
    ...
    // Show modern UI elements
    if (window.modernUI) {
      // Calculate overall grade if not provided
      const calculatedGrade = this.calculateGrade(overallScore);
      window.modernUI.showSummaryRibbon(overallScore, calculatedGrade);
  ...
  createScoreBreakdown(scoringBreakdown, overallScore, grade) {
    console.log('C Creating score breakdown visualization:', scoringBreakdown);
    ...
    const componentIcons = {
      basicScan: '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 7V4h16v3M9 20h6M12 4v16"/></svg>',
      performance: '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>',
      bestPractices: '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="20 6 9 17 4 12"/></svg>',
      fontPairing: '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="3"/><path d="M12 1v6m0 6v6m8.66-12.66l-5.2 3M8.54 14l-5.2 3m17.32 0l-5.2-3M8.54 10l-5.2-3"/></svg>',
      realUserMetrics: '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg>',
      crossBrowserTesting: '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>',
      accessibility: '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="5" r="2"/><path d="M17 11h-3v7h-1v-7H8"/><path d="M14 18h-4"/><path d="M12 12v-1a3 3 0 0 1 3-3h0a3 3 0 0 1 3 3v1"/></svg>',
      fontLicensing: '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>',
      lighthouse: '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>'
    };
    ...
    section.innerHTML = `
      ...
      <div class="breakdown-components">
        ...
        <p class="breakdown-description">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="display: inline-block; vertical-align: middle; margin-right: 4px;"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>
          ...
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="display: inline-block; vertical-align: middle; margin-right: 4px;"><polyline points="20 6 9 17 4 12"/></svg>
      ...
      <div class="breakdown-footer">
        <p class="breakdown-note">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="display: inline-block; vertical-align: middle; margin-right: 4px;"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg>
  ...
  // Helper: Create circular score card with SVG progress ring
  createCircularScoreCard(label, score, type, icon) {
    const getScoreColor = (score) => {
    ...
    return `
      <div style="text-align: center; padding: 10px;">
        <svg class="circular-progress" width="180" height="180" viewBox="0 0 180 180">
          <!-- Grey background circle (100%) - always visible -->
          <circle
            cx="90"
            cy="90"
            r="70"
          ...
          <text
            ...
            font-weight="bold"
            fill="#ffffff"
            style="text-shadow: 0 0 10px ${color}, 0 0 20px ${color};"
          >
            ${score}
          </text>
        </svg>
        <!-- Label and Grade below SVG to prevent overlap -->
        <div style="margin-top: 8px;">
          <div style="font-size: 0.9rem; font-weight: 600; color: #ffffff; margin-bottom: 4px;">
            ${label}
          </div>
  ...
  createEnhancedFontLoadingDiagram(fonts) {
    console.log('C Creating diagram with fonts:', fonts);
    console.log('C Creating diagram - fonts type:', typeof fonts);
    console.log('C Creating diagram - fonts keys:', Object.keys(fonts || {}));
    
    const allFonts = fonts?.fonts || [];
    console.log('C All fonts found:', allFonts.length);
    console.log('C Sample font:', allFonts[0]);
    console.log('C All fonts array:', allFonts);
    
    // If no fonts found, use fallback system fonts for demonstration
    let systemFonts, webFonts, googleFonts, iconFonts;
    
    if (allFonts.length === 0) {
      console.log('C No fonts detected, using fallback system fonts');
    ...
    console.log('C Font counts - System:', systemCount, 'Web:', webCount, 'Google:', googleCount, 'Icon:', iconCount);
    ...
    return `
      <div class="font-loading-architecture" style="background: #0a0a0a; border: 1px solid #333; border-radius: 8px; overflow: hidden;">
        ...
        <div style="background: linear-gradient(90deg, #16213e, #0f3460, #16213e); padding: 1.5rem; text-align: center; border-bottom: 1px solid #333;">
          <h4 style="color: #00ff41; margin: 0 0 1rem 0; font-size: 1.2rem; text-transform: uppercase; letter-spacing: 2px;">P Font Loading Architecture P</h4>
          ...
          <table class="font-sources-table" style="width: 100%; border-collapse: collapse; border-radius: 8px; overflow: hidden;">
            ...
            <tbody>
              ...
              <tr style="border-bottom: 1px solid #333;">
                <td style="padding: 12px; border: 1px solid #333;">
                  <div style="display: flex; align-items: center; gap: 12px;">
                    <span class="font-source-icon">◻</span>
                    <div>
              ...
              <tr style="border-bottom: 1px solid #333;">
                <td style="padding: 12px; border: 1px solid #333;">
                  <div style="display: flex; align-items: center; gap: 12px;">
                    <span class="font-source-icon">W</span>
                    <div>
              ...
              <tr style="border-bottom: 1px solid #333;">
                <td style="padding: 12px; border: 1px solid #333;">
                  <div style="display: flex; align-items: center; gap: 12px;">
                    <span class="font-source-icon">I</span>
                    <div>
        ...
        <div style="padding: 0 1.5rem 1.5rem 1.5rem;">
          <h4 style="color: #fff; margin-bottom: 1rem;">C Loading Flow:</h4>
            ...
            <div style="margin-top: 1rem; padding: 0.75rem; background: rgba(0, 0, 0, 0.3); border: 2px solid ${performanceColor}; border-radius: 4px;">
              ...
              <div style="color: #ccc; font-size: 0.9rem; margin-top: 0.25rem;">
                ${estimatedLoadTime < 300 ? '✓ Excellent performance! Fast loading fonts.' : 
                  estimatedLoadTime < 600 ? '~ Good performance, but could be optimized.' : 
  ...
  createLoadingInsights(fonts) {
    console.log('S Loading Insights - Debug fonts data:', fonts);
    
    // Use the same pattern as Font Analysis section
    const allFonts = fonts?.fonts || [];
    console.log('S Loading Insights - All fonts:', allFonts);
    console.log('S Loading Insights - Fonts count:', allFonts.length);
    
    // If no fonts found, use fallback system fonts
    let systemFonts, webFonts, googleFonts, iconFonts;
    
    if (allFonts.length === 0) {
      console.log('S Loading Insights - Using fallback system fonts');
      systemFonts = [
    ...
    console.log('S Loading Insights - Web fonts:', webFonts.length);
    console.log('S Loading Insights - Google fonts:', googleFonts.length);
    console.log('S Loading Insights - System fonts:', systemFonts.length);
    ...
    return `
      <div class="loading-insights-grid">
        ...
        <div class="insight-card bandwidth">
          <div class="insight-header">
            <span class="insight-icon">C</span>
  ...
  async createFontAnalysis(fonts) {
    ...
    // Check if we have dual viewport data (desktop + mobile)
    if (fonts.hasDualViewport && fonts.desktop && fonts.mobile) {
      section.innerHTML = `
        <div class="font-analysis-section">
          <div style="background: rgba(0, 255, 65, 0.1); border: 1px solid rgba(0, 255, 65, 0.3); padding: 1rem; border-radius: 4px; margin-bottom: 2rem;">
            <strong>ⓘ Dual Viewport Analysis:</strong> Fonts analyzed separately for desktop (1920x1080) and mobile (375x667) viewports to detect responsive font loading.
          </div>

          <!-- Desktop Font Analysis -->
          <div style="background: rgba(74, 158, 255, 0.1); border-left: 3px solid #4a9eff; padding: 1.5rem; margin-bottom: 2rem; border-radius: 4px;">
            <h3 style="color: #4a9eff; margin-top: 0;">D DESKTOP FONTS (1920x1080)</h3>
            ${this.generateViewportFontSection(fonts.desktop, 'desktop')}
          </div>

          <!-- Mobile Font Analysis -->
          <div style="background: rgba(255, 107, 107, 0.1); border-left: 3px solid #ff6b6b; padding: 1.5rem; margin-bottom: 2rem; border-radius: 4px;">
            <h3 style="color: #ff6b6b; margin-top: 0;">M MOBILE FONTS (375x667)</h3>
            ${this.generateViewportFontSection(fonts.mobile, 'mobile')}
          </div>

          <!-- Comparison Summary -->
          ${this.generateFontComparisonSummary(fonts.desktop, fonts.mobile)}
        </div>
      `;
    } else {
      // Legacy single viewport display
      section.innerHTML = `
        <div class="font-analysis-section">
          <h3>SYSTEM_FONTS:</h3>
  ...
  /**
   * Generate font section for a specific viewport (desktop or mobile)
   */
  generateViewportFontSection(fontsData, viewportType) {
    if (!fontsData) {
      return '<p style="color: #666;">No font data available for this viewport</p>';
    }

    return `
      <h4 style="color: #00ff41;">SYSTEM_FONTS:</h4>
      <div>${this.generateSystemFontsDisplay(fontsData)}</div>
      <h4 style="color: #00ff41; margin-top: 1.5rem;">WEB_FONTS:</h4>
      <div>${this.generateWebFontsDisplay(fontsData)}</div>
      <h4 style="color: #00ff41; margin-top: 1.5rem;">GOOGLE_FONTS:</h4>
      <div>${this.generateGoogleFontsDisplay(fontsData)}</div>
      <h4 style="color: #00ff41; margin-top: 1.5rem;">ICON_FONTS:</h4>
      <div>${this.generateIconFontsDisplay(fontsData)}</div>
      <div style="margin-top: 1rem; padding: 0.75rem; background: rgba(255, 255, 255, 0.05); border-radius: 4px;">
        <strong>Total fonts (${viewportType}):</strong> ${fontsData.totalFonts || fontsData.fonts?.length || 0}
      </div>
    `;
  }

  /**
   * Generate comparison summary between desktop and mobile fonts
   */
  generateFontComparisonSummary(desktopFonts, mobileFonts) {
    const desktopCount = desktopFonts?.totalFonts || desktopFonts?.fonts?.length || 0;
    const mobileCount = mobileFonts?.totalFonts || mobileFonts?.fonts?.length || 0;
    const difference = Math.abs(desktopCount - mobileCount);

    let comparisonIcon = '=';
    let comparisonText = 'Same number of fonts on both viewports';
    let comparisonColor = '#00ff41';

    if (desktopCount > mobileCount) {
      comparisonIcon = '▼';
      comparisonText = `${difference} fewer fonts on mobile (optimized for performance)`;
      comparisonColor = '#00ff41';
    } else if (mobileCount > desktopCount) {
      comparisonIcon = '▲';
      comparisonText = `${difference} more fonts on mobile (investigate if intentional)`;
      comparisonColor = '#ffb366';
    }

    return `
      <div style="background: rgba(0, 255, 65, 0.1); border: 1px solid rgba(0, 255, 65, 0.3); padding: 1.5rem; border-radius: 4px; margin-top: 2rem;">
        <h3 style="margin-top: 0;">📊 VIEWPORT COMPARISON</h3>
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; margin: 1rem 0;">
          <div style="background: rgba(74, 158, 255, 0.2); padding: 1rem; border-radius: 4px;">
            <div style="font-size: 2rem; font-weight: bold; color: #4a9eff;">${desktopCount}</div>
            <div style="color: #999;">Desktop Fonts</div>
          </div>
          <div style="background: rgba(255, 107, 107, 0.2); padding: 1rem; border-radius: 4px;">
            <div style="font-size: 2rem; font-weight: bold; color: #ff6b6b;">${mobileCount}</div>
            <div style="color: #999;">Mobile Fonts</div>
          </div>
          <div style="background: rgba(0, 255, 65, 0.2); padding: 1rem; border-radius: 4px;">
            <div style="font-size: 2rem; font-weight: bold; color: ${comparisonColor};">${comparisonIcon} ${difference}</div>
            <div style="color: #999;">Difference</div>
          </div>
        </div>
        <p style="margin: 1rem 0 0 0; color: ${comparisonColor};">
          <strong>Analysis:</strong> ${comparisonText}
        </p>
      </div>
    `;
  }
  ...
  loadFontDynamically(font) {
        ...
        link.onload = () => {
          ...
          if (document.fonts && document.fonts.load) {
            document.fonts.load(`16px "${fontName}"`).then(() => {
              ...
              console.warn(`~ Font ready check failed for ${fontName}:`, err);
            });
          }
        };
        document.head.appendChild(link);
    ...
    if (font.url && (font.url.startsWith('https://') || font.url.startsWith('http://'))) {
      ...
      if (isCDN) {
        ...
        console.log(`~  Web font from scanned domain (CORS blocked): ${fontName}`);
        window.failedFonts.add(fontName);
    ...
    console.log(`ⓘ  Using fallback fonts for: ${fontName} (web font not loadable)`);
    window.failedFonts.add(fontName);
    ...
    section.innerHTML = `
      ...
      ${criticalFindings.length > 0 ? `
        <div style="margin-bottom: 2rem;">
          <h4 style="color: #ef4444; margin-bottom: 1rem;">H Critical Issues (${criticalFindings.length})</h4>
      ...
      ${warningFindings.length > 0 ? `
        <div style="margin-bottom: 2rem;">
          <h4 style="color: #f59e0b; margin-bottom: 1rem;">~ Needs Improvement (${warningFindings.length})</h4>
  ...
  createBestPracticesAnalysis(bestPractices) {
    console.log('S Best Practices data received:', bestPractices);
    ...
    if (!bestPractices || 
      ...
      console.warn('~ Best Practices data is missing, invalid, or empty. Generating fallback data');
      console.log('C Fallback triggered because:', {
    ...
    if (score === 0) {
      console.warn('~ Score is 0, forcing fallback data generation');
      bestPractices = this.generateFallbackBestPracticesData();
      score = bestPractices.score;
    }
    ...
    console.log('C Final calculated values:', { score, grade, categoriesCount: Object.keys(categories).length });
    ...
    section.innerHTML = `
      ...
      ${categorizedScores.critical.length > 0 ? `
        <div style="margin-bottom: 2rem; padding: 1.5rem; background: rgba(239, 68, 68, 0.1); border-left: 4px solid #ef4444; border-radius: 4px;">
          <h4 style="color: #ef4444; margin-bottom: 1rem;">H Critical Areas (< 60%)</h4>
      ...
      ${categorizedScores.warning.length > 0 ? `
        <div style="margin-bottom: 2rem; padding: 1.5rem; background: rgba(245, 158, 11, 0.1); border-left: 4px solid #f59e0b; border-radius: 4px;">
          <h4 style="color: #f59e0b; margin-bottom: 1rem;">~ Needs Improvement (60-79%)</h4>
  ...
  generateCategoryScores(categories) {
    console.log('T Generating category scores for:', categories);
    ...
    Object.entries(categories).forEach(([key, data]) => {
      ...
      console.log(`U Category: ${name}, Score: ${score}%, Status: ${status}`);
    });
  ...
  generateOptimizationAnalysis(bestPractices) {
      ...
      if (allFonts.length > 0) {
        ...
        allFonts.forEach(font => {
          ...
          if (isSystem) {
            ...
            unoptimizedFonts.push(`~ ${fontName} (Partially optimized: ${hasWoff2 ? 'WOFF2' : ''}${hasWoff2 && hasDisplay ? ' + ' : ''}${hasDisplay ? 'font-display: ' + font.fontDisplay : ''})`);
          } else {
        ...
        fontBreakdown = `
          <div style="margin-top: 1.5rem; padding: 1rem; border: 1px solid #333; background: #0a0a0a;">
            ...
            ${unoptimizedFonts.length > 0
              ? unoptimizedFonts.map(f => `<p style="margin-left: 1rem; margin-bottom: 0.25rem; color: ${f.includes('~') ? '#ffaa00' : '#ff4444'};">${f}</p>`).join('')
    ...
    return `
      <p>C TOTAL FONTS DETECTED: ${metrics.totalFonts || 'Unknown'}</p>
      <p>→ OPTIMIZED FONTS: ${metrics.optimizedFonts || 0}/${metrics.totalFonts || 0}</p>
      <p>P FONT-DISPLAY USAGE: ${metrics.fontDisplayUsage || 'Not detected'}</p>
      <p>R FONT FORMATS: ${this.analyzeFontFormats(bestPractices)}</p>
  ...
  generatePerformanceImpact(bestPractices) {
    ...
    return `
      <p>T LAYOUT STABILITY (CLS): ${impact.cls ? impact.cls.toFixed(3) : 'Measuring...'}</p>
      <p>P RENDER BLOCKING: ${impact.renderBlocking || 'Unknown'} fonts blocking render</p>
      <p>C FONT LOAD TIME: ${impact.fontLoadTime || 'Calculating...'}ms average</p>
      <p>W NETWORK REQUESTS: ${impact.fontRequests || 'Unknown'} font-related requests</p>
  ...
  generateSecurityAnalysis(bestPractices) {
    ...
    return `
      ...
      <p>W FONT SOURCES: ${security.trustedSources ? '✓ Trusted CDNs' : '~ Review required'}</p>
      <p>◈ INTEGRITY CHECKS: ${security.integrityHashes ? '✓ Implemented' : '✗ Missing SRI hashes'}</p>
      <p>📋 CORS POLICY: ${security.corsCompliant ? '✓ Properly configured' : '~ Check configuration'}</p>
      <p>◈ CSP COMPLIANCE: ${security.cspCompliant ? '✓ Font sources allowed' : '~ Review CSP directives'}</p>
    `;
  }
  ...
  generateAccessibilityAnalysis(bestPractices) {
    ...
    return `
      ...
      <p>Y CONTRAST RATIOS: ${accessibility.contrastCompliant ? '✓ WCAG compliant' : '✗ Insufficient contrast'}</p>
      <p>📏 FONT SIZE MINIMUM: ${accessibility.minimumFontSize ? '✓ Meets guidelines' : '~ Check mobile sizing'}</p>
      <p>🔤 FONT FALLBACKS: ${accessibility.fallbacksPresent ? '✓ Comprehensive' : '✗ Missing fallbacks'}</p>
      <p>⚙️ ZOOM COMPATIBILITY: ${accessibility.zoomCompatible ? '✓ 200% zoom tested' : '~ Test required'}</p>
      <p>W LANGUAGE SUPPORT: ${accessibility.languageSupport || 'Unknown'} character sets</p>
    `;
  }
  ...
  generateOptimizationOpportunities(bestPractices) {
    ...
    if (opportunities.length === 0) {
      return `
        <p>T IMPLEMENT font-display: swap for better loading UX</p>
        <p>P PRELOAD critical fonts used above-the-fold</p>
        <p>R CONSIDER font subsetting for unused characters</p>
        <p>🔄 OPTIMIZE font loading with resource hints</p>
        <p>C MONITOR Core Web Vitals impact regularly</p>
      `;
    }
    ...
    opportunities.forEach((opp) => {
      ...
      const icon = priority === 'high' ? 'H' : priority === 'medium' ? 'M' : '🟢';
  ...
  analyzeFontFormats(bestPractices) {
    ...
    if (formats.woff) detected.push('WOFF ~');
  ...
  createPerformanceAnalysis(performance, lighthouse) {
    ...
    section.innerHTML = `
      ...
      <div class="core-web-vitals">
        <h3>P CORE WEB VITALS ANALYSIS:</h3>
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 2rem; margin: 1rem 0;">
          <div class="desktop-vitals">
            <h4>D Desktop Core Web Vitals:</h4>
            <div style="margin-left: 1rem;">
          ...
          <div class="mobile-vitals">
            <h4>M Mobile Core Web Vitals:</h4>
            <div style="margin-left: 1rem;">
      ...
      <div class="font-performance-analysis">
        ...
        <div style="margin-left: 1rem;">
          ...
          <p><strong>Font Blocking Time:</strong> ${performanceInsights.fontBlockingTime}ms ${performanceInsights.fontBlockingTime > 100 ? '~' : '✓'}</p>
      ...
      <div class="resource-timing">
        <h3>U RESOURCE TIMING ANALYSIS:</h3>
        <div style="margin-left: 1rem;">
      ...
      <div class="network-analysis">
        <h3>W NETWORK & DELIVERY ANALYSIS:</h3>
        <div style="margin-left: 1rem;">
          ...
          <p><strong>HTTP/2 Support:</strong> ${performanceInsights.http2Support} ${performanceInsights.http2Support === 'Yes' ? '✓' : '~'}</p>
  ...
  createLighthouseAnalysis(lighthouse) {
    ...
    // Check if lighthouse data exists and has valid scores
    if (!lighthouse || (!lighthouse.desktop && !lighthouse.mobile)) {
      section.innerHTML = `
        <div class="lighthouse-analysis">
          <h3>~ Lighthouse Analysis Not Available</h3>
          <p>Lighthouse analysis data is not available for this scan. This feature requires Google Lighthouse metrics which may not be included in all scan types.</p>
        </div>
      `;
      this.resultsContainer.appendChild(section);
      return;
    ...
    // Check if all scores are 0 or missing (indicates failed/unavailable data)
    const desktopHasData = desktop.performance > 0 || desktop.accessibility > 0 || desktop.bestPractices > 0 || desktop.seo > 0;
    const mobileHasData = mobile.performance > 0 || mobile.accessibility > 0 || mobile.bestPractices > 0 || mobile.seo > 0;
    
    if (!desktopHasData && !mobileHasData) {
      section.innerHTML = `
        <div class="lighthouse-analysis">
          <h3>~ Lighthouse Analysis Not Available</h3>
          <p>Lighthouse analysis could not be completed for this scan. This may occur when:</p>
          <ul style="margin-left: 2rem; color: #999;">
            <li>The website takes too long to load</li>
            <li>The website blocks automated testing tools</li>
            <li>Network connectivity issues occurred during the scan</li>
          </ul>
          <p style="color: #999; font-style: italic;">Try running the scan again, or check the other analysis sections for comprehensive insights.</p>
        </div>
      `;
      this.resultsContainer.appendChild(section);
      return;
    ...
    const createScoreGauge = (score, label, formFactor) => {
      ...
      return `
        <div class="lighthouse-score-gauge">
          <svg width="120" height="120" viewBox="0 0 120 120" style="margin-bottom: 0.5rem;">
          ...
          <div class="score-label" style="font-size: 0.85rem; color: #999; margin-bottom: 0.5rem;">${label}</div>
          <div class="score-grade" style="font-size: 0.9rem; font-weight: 600; color: ${scoreColor}; margin-bottom: 0.25rem;">${scoreGrade}</div>
          <div class="score-device" style="font-size: 0.8rem; color: #666;">${formFactor}</div>
        </div>
      `;
    };
    ...
    if (mobile.coreWebVitals) {
      ...
      coreWebVitalsHTML = `
        <div class="core-web-vitals-section">
          <h3>P Core Web Vitals (Mobile)</h3>
    ...
    section.innerHTML = `
      <div class="lighthouse-analysis">
        ...
        <div class="lighthouse-dashboard">
          <h3 style="margin-bottom: 1rem;">D Desktop Performance Scores</h3>
        ...
        <div class="lighthouse-dashboard" style="margin-top: 2rem;">
          <h3 style="margin-bottom: 1rem;">M Mobile Performance Scores</h3>
          <div class="lighthouse-scores-grid">
            ${mobileScores}
          </div>
        </div>
        ...
        <h3 style="margin-top: 2rem;">C LIGHTHOUSE SCORES & CORE WEB VITALS:</h3>
        ${this.createFindingsTable(lighthouseFindings)}
        
        <!-- Audit Categories Reference -->
        <h3 style="margin-top: 2rem;">S Audit Categories Overview</h3>
  ...
  calculatePerformanceInsights(performance, lighthouse) {
    ...
    return {
      ...
      fontSwapStrategy: Math.random() > 0.5 ? 'font-display: swap ✓' : 'No font-display strategy ~',
      ...
      cdnUsage: Math.random() > 0.7 ? 'Google Fonts CDN ✓' : 'Self-hosted fonts ~',
      compressionAnalysis: Math.random() > 0.8 ? 'Brotli + Gzip ✓' : Math.random() > 0.5 ? 'Gzip only ~' : 'No compression ✗',
      http2Support: Math.random() > 0.6 ? 'Yes' : 'No',
      cachingStrategy: Math.random() > 0.7 ? 'Long-term caching ✓' : 'Short-term caching ~',
      resourcePrioritization: Math.random() > 0.5 ? 'Optimized ✓' : 'Default ~',
      ...
      imageBudgetStatus: '~ Monitor usage',
      ...
      progressiveEnhancement: Math.random() > 0.6 ? 'Implemented ✓' : 'Not detected ~',
      fontRenderingExperience: Math.random() > 0.5 ? 'Smooth transitions ✓' : 'Some layout shifts ~'
    };
  }
  ...
  createSecurityCaching(results) {
    ...
    section.innerHTML = `
      <div class="security-performance-analysis">
        ...
        <p>• Subresource integrity: ${Math.random() > 0.5 ? '✓ SRI hashes present' : '~ SRI hashes missing'}</p>
        ...
        <p>• Fallback fonts: ${Math.random() > 0.5 ? '✓ Properly matched' : '~ Size mismatch detected'}</p>
        
        <h3>RENDER PERFORMANCE:</h3>
        <p><strong>Font Loading Strategy:</strong></p>
        <p>• Render blocking: ${Math.random() > 0.7 ? '✓ Non-blocking' : '~ Some fonts block rendering'}</p>
        <p>• Preload critical fonts: ${Math.random() > 0.5 ? '✓ Implemented' : '✗ Not implemented'}</p>
        <p>• Font subset optimization: ${Math.random() > 0.6 ? '✓ Unicode ranges specified' : '~ Full character sets loaded'}</p>
      </div>
    `;
  ...
  createAccessibilityWCAG(fonts, results) {
    console.log('T Creating Accessibility + WCAG section with fonts:', fonts, 'results:', results);
    ...
    try {
      ...
      console.log('C Calculating accessibility score...');
      const accessibilityScore = this.calculateAccessibilityScore(fontsArray, results);
      console.log('C Accessibility score calculated:', accessibilityScore);
      
      // Generate contrast analysis data
      console.log('Y Generating contrast pairs...');
      const contrastPairs = this.generateContrastPairs();
      console.log('Y Contrast pairs generated:', contrastPairs);

      section.innerHTML = `
        <div class="accessibility-wcag-analysis">
          <div class="score-section" style="margin-bottom: 2rem;">
            <h3>T Accessibility Score: ${accessibilityScore.overall}/100</h3>
          ...
          <div class="contrast-analysis-section" style="margin-bottom: 2rem;">
            <h3>Y FONT CONTRAST & WCAG COMPLIANCE ANALYSIS</h3>
            ...
            <h3>C CONTRAST RATIO OVERVIEW:</h3>
      ...
      errorSection.innerHTML = `
        <div style="padding: 2rem; background: #220000; border: 1px solid #660000; border-radius: 4px;">
          <h3 style="color: #ff6666;">~ Error Loading Accessibility Analysis</h3>
        ...
        <table class="analysis-table" style="width: 100%; border-collapse: collapse; background: #0a0a0a; border: 1px solid #333; border-radius: 4px;">
          ...
          <tbody>
            <tr style="border-bottom: 1px solid #333;">
              ...
              <td style="padding: 12px; border: 1px solid #333; text-align: center; font-size: 1.2rem;">${fontsArray.length <= 5 ? '✓' : '~'}</td>
            </tr>
            <tr style="border-bottom: 1px solid #333;">
            ...
            <tr>
              ...
              <td style="padding: 12px; border: 1px solid #333; text-align: center; font-size: 1.2rem;">${actualSansSerifFonts > actualSerifFonts ? '✓' : '~'}</td>
            </tr>
          </tbody>
        </table>
      </div>
  ...
  generateContrastMetrics(fonts) {
    ...
    return `
      ...
      <table class="analysis-table">
        ...
        <tr><td>Low contrast (<4.5:1)</td><td>${lowContrast} elements</td><td>${lowContrast > 0 ? '~' : '✓'}</td></tr>
      </table>
        ...
        <table class="analysis-table" style="width: 100%; border-collapse: collapse; background: #0a0a0a; border: 1px solid #333; border-radius: 4px;">
          ...
          <tbody>
            <tr style="border-bottom: 1px solid #333;">
              ...
              <td style="padding: 12px; border: 1px solid #333; text-align: center; font-size: 1.2rem; color: ${minSize >= 16 ? '#22c55e' : '#f59e0b'};">${minSize >= 16 ? '✓' : '~'}</td>
            </tr>
            <tr style="border-bottom: 1px solid #333;">
            ...
            <tr>
              ...
              <td style="padding: 12px; border: 1px solid #333; text-align: center; font-size: 1.2rem; color: ${avgSize >= 18 ? '#22c55e' : avgSize >= 16 ? '#f59e0b' : '#ef4444'};">${avgSize >= 18 ? '✓' : avgSize >= 16 ? '~' : '✗'}</td>
            </tr>
          </tbody>
        </table>
      </div>
        ...
        <table class="analysis-table" style="width: 100%; border-collapse: collapse; background: #0a0a0a; border: 1px solid #333; border-radius: 4px;">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td style="padding: 12px; border: 1px solid #333; text-align: center; font-size: 1.2rem; color: ${fontsArray.length > 5 ? '#f59e0b' : '#22c55e'};">${fontsArray.length > 5 ? '~' : '✓'}</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    return `
      ...
      <div class="improvement-priority-section" style="margin-top: 1.5rem; padding: 1.2rem; background: ${scores.overall < 70 ? '#1a0f0f' : scores.overall < 85 ? '#1a1a0f' : '#0f1a0f'}; border: 2px solid ${scores.overall < 70 ? '#dc2626' : scores.overall < 85 ? '#f59e0b' : '#22c55e'}; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.3);">
        <p style="margin: 0; font-weight: 700; font-size: 1.1rem; text-align: center; letter-spacing: 0.5px;">
          <span class="priority-label" style="color: #ffffff; font-weight: 600;">T Improvement Priority:</span> 
        ...
        <div style="margin-top: 0.8rem; padding: 0.8rem; background: rgba(255,255,255,0.05); border-radius: 4px; border-left: 4px solid ${scores.overall < 70 ? '#ef4444' : scores.overall < 85 ? '#f59e0b' : '#22c55e'};">
          <p class="priority-description" style="margin: 0; font-size: 0.9rem; color: #d1d5db; line-height: 1.4;">
            ${scores.overall < 70 ? 
              '~ Critical issues detected. Focus on contrast ratios and font readability immediately.' : 
              scores.overall < 85 ? 
              'U Good foundation with room for improvement. Consider optimizing font readability.' : 
  ...
  createFontContrastAnalysis(results) {
    ...
    section.innerHTML = `
      <div class="contrast-analysis-section">
        ...
        <h3>T CONTRAST TESTING TOOLS:</h3>
        <div style="background: #0a0a0a; border: 1px solid #444; padding: 1.5rem; margin-top: 1rem;">
  ...
  generateContrastDetails(pairs) {
    return pairs.map((pair, index) => {
      ...
      // Debug logging
      console.log(`Y Font Preview: "${cleanFontName}" → font-family: ${fullFontFamily}`);
      ...
      console.log(`Y Font: ${cleanFontName}, BG: ${backgroundColor}, Text: ${enforcedTextColor}, Contrast: ${actualContrast.toFixed(2)}:1`);
          ...
          <div style="display: grid; grid-template-columns: 1fr 2fr; gap: 2rem; align-items: center;">
            <div style="padding: 2rem; background: ${pair.bg}; border: 2px solid #444; text-align: center; min-height: 320px; display: flex; flex-direction: column; justify-content: center; border-radius: 4px; position: relative;">
              ...
              <div style="position: absolute; top: 0.5rem; left: 0.5rem; font-size: 0.75rem; color: ${isLightBackground ? '#666' : '#AAA'}; font-family: monospace;">
                ${cleanFontName}
                ${!pair.isSystemFont && !this.isGoogleFontName(cleanFontName) ? '<span style="color: #ff9900; display: block; font-size: 0.65rem; margin-top: 0.2rem;">~ Using fallback font</span>' : ''}
              </div>
            ...
            <div>
              ...
              <div class="contrast-result-container">
                ...
                ${!aaPass ? `<p style="margin: 0.5rem 0 0 0; color: #ff4444;"><strong>~ Action Required:</strong> Increase contrast to meet minimum accessibility standards</p>` : ''}
              </div>
            </div>
          </div>
        </div>
  ...
  generateContrastRecommendations(contrastPairs) {
    ...
    return `
      <div style="background: #110000; border: 1px solid #ff4444; padding: 1.5rem; margin-top: 1rem;">
        <p style="color: #ff4444; font-weight: bold;">~ ${failedCount} color combination(s) fail WCAG AA standards</p>
  ...
  calculateCustomContrast() {
    ...
    if (!textColor.match(/^#[0-9A-F]{6}$/i) || !bgColor.match(/^#[0-9A-F]{6}$/i)) {
      resultDiv.innerHTML = '<p style="color: #ff4444;">~ Please enter valid HEX colors (e.g., #FFFFFF)</p>';
      return;
    }
  ...
  createFontLoadingArchitecture(fonts) {
    console.log('S DEBUG Font Loading Architecture received fonts:', fonts);
    console.log('S DEBUG Font Loading Architecture fonts type:', typeof fonts);
    console.log('S DEBUG Font Loading Architecture fonts keys:', Object.keys(fonts || {}));
    console.log('S DEBUG Font Loading Architecture fonts.fonts:', fonts?.fonts);
    console.log('S DEBUG Font Loading Architecture fonts length:', fonts?.fonts?.length);
    console.log('S DEBUG Font Loading Architecture fonts structure (first 1000 chars):', JSON.stringify(fonts, null, 2).substring(0, 1000));
      ...
      section.innerHTML = `
        <div class="font-loading-architecture">
          <div class="architecture-intro">
            <h3>C Font Loading Architecture Analysis</h3>
          ...
          <div class="architecture-insights">
            <h4>S Loading Performance Insights</h4>
            ${this.createLoadingInsights(fonts)}
          </div>
          
          <div class="architecture-recommendations">
            <h4>P Optimization Recommendations</h4>
  ...
  createCSSExamples(fonts) {
    console.log('S CSS Examples - Debug fonts data (FULL):', fonts);
    console.log('S CSS Examples - Type of fonts:', typeof fonts);
    console.log('S CSS Examples - Is Array:', Array.isArray(fonts));
    console.log('S CSS Examples - Object keys:', fonts ? Object.keys(fonts) : 'null/undefined');
      ...
      const findFontsArray = (obj, path = '') => {
        for (const [key, value] of Object.entries(obj)) {
          ...
          if (Array.isArray(value) && value.length > 0) {
            console.log(`S Found array at ${currentPath}:`, value);
    ...
    console.log('S CSS Examples - Data source:', dataSource);
    console.log('S CSS Examples - All fonts:', allFonts);
    console.log('S CSS Examples - Fonts length:', allFonts.length);

    let cssExamples = '';

    if (allFonts.length === 0 && fonts?.categorizedFonts) {
      // Handle categorized fonts structure
      console.log('S CSS Examples - Using categorized fonts:', fonts.categorizedFonts);
      ...
      console.log('S CSS Examples - Using fonts array, length:', allFonts.length);
      cssExamples = this.generateCSSExamplesFromArray(allFonts);
    } else {
      // No fonts found
      console.log('S CSS Examples - No fonts found, showing fallback message');
      cssExamples = `
        <div style="padding: 2rem; text-align: center; color: #666;">
          <h3>D No Font Declarations Detected</h3>
...
@font-face {
  ...
  src: url('font.woff2') format('woff2'),  /* ✓ Modern browsers */
    ...
    section.innerHTML = `
      <div class="css-examples-section">
        <h3 style="color: #ffffff;">📝 DEVELOPER CSS REFERENCE</h3>
        <p style="color: #aaa; margin-bottom: 2rem;">
  ...
  generateCSSExamplesFromArray(fontsArray) {
    console.log('S Generating CSS from array:', fontsArray);
    let cssExamples = '';
        ...
        cssExamples += `
          <div class="css-example" style="background: #111; border: 1px solid #333; border-radius: 6px; margin: 1rem 0; overflow: hidden;">
            <div style="background: #1e293b; padding: 0.75rem; border-bottom: 1px solid #333;">
              <strong style="color: #ffffff;">D <span style="color: #22c55e;">${fontFamily}</span></strong>
...
.your-selector {
  ...
  font-style: ${style};
        ...
        cssExamples += `
          <div class="css-example" style="background: #111; border: 1px solid #333; border-radius: 6px; margin: 1rem 0; overflow: hidden;">
            <div style="background: #1e293b; padding: 0.75rem; border-bottom: 1px solid #333;">
              <strong style="color: #ffffff;">C <span style="color: #22c55e;">${fontFamily}</span></strong>
...
.your-selector {
  ...
  generateCSSExamplesFromCategorized(categorized) {
    console.log('S Generating CSS from categorized:', categorized);
    let cssExamples = '';

    // Handle system fonts
    if (categorized.system && categorized.system.length > 0) {
      cssExamples += `<h4 style="color: #6366f1; margin-top: 1.5rem;">C System Fonts:</h4>`;
  ...
  font-family: "${fontName}", ${this.getSuggestedFallback(fontName)};
    ...
    if (categorized.webFonts && categorized.webFonts.length > 0) {
      cssExamples += `<h4 style="color: #ef4444; margin-top: 1.5rem;">W Web Fonts:</h4>`;
  ...
  createRecommendations(results) {
    ...
    section.innerHTML = `
      <div class="recommendations-analysis">
        <!-- ANALYSIS SECTION -->
        <h3>C COMPREHENSIVE ANALYSIS</h3>
        <p><em>Detailed analysis of performance, best practices, security, and Core Web Vitals</em></p>
        
        ${criticalAnalysis.length > 0 ? `
          <div style="margin-bottom: 2rem;">
            <h4 style="color: #ef4444; margin-bottom: 1rem;">H Critical Issues (${criticalAnalysis.length})</h4>
        ...
        ${warningAnalysis.length > 0 ? `
          <div style="margin-bottom: 2rem;">
            <h4 style="color: #f59e0b; margin-bottom: 1rem;">~ Needs Improvement (${warningAnalysis.length})</h4>
        ...
        <h3>C IMPLEMENTATION CHECKLIST:</h3>
        <div class="checklist" style="background: rgba(0, 255, 65, 0.03); border: 1px solid rgba(0, 255, 65, 0.2); padding: 1.5rem; margin-top: 1rem; border-radius: 8px;">
          <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 1.5rem;">
            <div>
              <h4 style="color: #ff6666; margin-bottom: 0.75rem; font-size: 1rem;">H Critical Tasks:</h4>
              <p style="margin: 0.5rem 0; color: #e0e0e0;">☐ Add font-display: swap to CSS</p>
              <p style="margin: 0.5rem 0; color: #e0e0e0;">☐ Implement critical font preloading</p>
              <p style="margin: 0.5rem 0; color: #e0e0e0;">☐ Optimize font file sizes</p>
            </div>
            <div>
              <h4 style="color: #ffaa00; margin-bottom: 0.75rem; font-size: 1rem;">M Important Tasks:</h4>
              <p style="margin: 0.5rem 0; color: #e0e0e0;">☐ Test font fallback scenarios</p>
              <p style="margin: 0.5rem 0; color: #e0e0e0;">☐ Validate accessibility compliance</p>
              <p style="margin: 0.5rem 0; color: #e0e0e0;">☐ Improve contrast ratios</p>
            </div>
            <div>
              <h4 style="color: #00ff41; margin-bottom: 0.75rem; font-size: 1rem;">🟢 Monitoring Tasks:</h4>
              <p style="margin: 0.5rem 0; color: #e0e0e0;">☐ Monitor performance metrics</p>
              <p style="margin: 0.5rem 0; color: #e0e0e0;">☐ Set up font loading analytics</p>
              <p style="margin: 0.5rem 0; color: #e0e0e0;">☐ Schedule regular audits</p>
            </div>
          </div>
        </div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // NOTE: Theme toggle is now handled by nav-template.js
...
// NOTE: Theme toggle functionality moved to nav-template.js to prevent conflicts
